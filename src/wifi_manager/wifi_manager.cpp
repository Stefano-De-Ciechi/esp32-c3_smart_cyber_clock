...
<body>
  <div class="container">
    ...
    <form action="/add" method="post">
      ...
      <input type="text" name="password1" placeholder="Password WiFi 1">
      <select name="ssid2"></select>
      <input type="text" name="password2" placeholder="Password WiFi 2">
      <button type="submit">Apply and Restart</button>
    </form>
    <form action="/clear" method="post">
      <button type="submit" class="clear-btn">Delete all saved networks</button>
    </form>
  </div>
  ...
  function populateNetworks() {
    fetch('/scan')
      ...
      .catch(error => console.error('Network scan failed:', error));
  }
...
void AsyncWiFiManagerSimple::Setup() {
    ...
    if (!connectToSavedNetwork()) {
        Serial.println("[WiFi] Unable to connect to any of the saved networks.");
        startConfigMode();
    }
}
...
void AsyncWiFiManagerSimple::Setup(const char* ap_ssid, const char* ap_pass) {
    ...
    Serial.println("[AP] AP settings received dinamically:");
...
void AsyncWiFiManagerSimple::loop() {
    if (inConfigMode) {
        ...
        if (millis() - configStartTime > CONFIG_TIMEOUT * 1000) {
            Serial.println("[AP] Config timeout reached; Restarting...");
            ESP.restart();
        }
    } else {
        checkWiFiConnection();
...
void AsyncWiFiManagerSimple::checkWiFiConnection() {
    ...
    if (!isConnected) {
        ...
        if (now - lastReconnectAttempt >= 3000) {
            ...
            Serial.printf("[WiFi] 🔄 Trying to reconnect #%d...\n", reconnectAttempts);

            if (reconnectAttempts >= RECONNECT_ATTEMPTS) {
                Serial.println("[WiFi] ⛔ Too many reconnection attempts. Restarting...");
                ESP.restart();
            }

            if (!connectToSavedNetwork()) {
                Serial.println("[WiFi] ❌ None of your saved networks are available.");
            }
        }
    }
